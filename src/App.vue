    ...
    <div class="bg-white shadow-md rounded-3xl overflow-hidden letter-container">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div ref="cvContent" class="cv-content">
          <!-- Header -->
          <header
            class="bg-white rounded-2xl shadow-lg overflow-hidden mb-8 page-break-after"
            role="banner"
          ...
          >
            <div class="md:flex items-center gap-6 p-6 lg:p-8">
              ...
              <div class="text-center md:text-left flex-1 mt-6 md:mt-0">
                ...
                <p
                  class="mt-2 inline-block bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-1.5 rounded-full text-sm font-semibold shadow-sm"
                >
          ...
          <div
            :class="[
              ...
              { 'flex flex-col gap-8': isExporting },
            ]"
          >
            <!-- Sidebar -->
            <aside
              :class="['lg:col-span-1 space-y-6', { 'w-full': isExporting }]"
              aria-label="Sidebar"
              :style="{ position: isExporting ? 'static' : '' }"
            >
              <section
                class="bg-white p-5 rounded-2xl shadow-md page-break-inside"
                role="region"
                  ...
                  <div class="flex items-center gap-3" v-if="primaryContact.phone">
                    ...
                    >
                      <path
                        ...
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M3 5a2 2 0 012-2h2.586a1 1 0 01.707.293L12 6l3.707-2.707A1 1 0 0116.414 3H19a2 2 0 012 2v4a10 10 0 11-18 0V5z"
                      />
                    </svg>
                  ...
                  <div class="flex items-center gap-3" v-if="primaryContact.address">
                    ...
                    >
                      <path
                        ...
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M12 2C8.13 2 5 5.13 5 9c0 6.63 7 13 7 13s7-6.37 7-13c0-3.87-3.13-7-7-7z"
                      />
              ...
              >
                ...
                <div class="flex flex-wrap gap-2">
                  <span
                    ...
                    :key="skill"
                    class="text-xs bg-yellow-100 text-yellow-800 px-3 py-1.5 rounded-full font-medium"
                  >
                    {{ skill }}
                  </span>
                </div>
            ...
            <main
              :class="[
                'lg:col-span-2 mt-8 lg:mt-0 space-y-8',
                { 'w-full mt-0': isExporting },
              ]"
            ...
            >
              ...
              >
                ...
                <div class="grid grid-cols-1 gap-3 text-sm text-gray-700">
                  <div class="flex justify-between">
                    <span class="font-medium">Full Name</span><span>{{ aboutMe.fullName }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="font-medium">Date of Birth</span><span>{{ aboutMe.dob }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="font-medium">Place of Birth</span><span>{{ aboutMe.pob }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="font-medium">Gender</span><span>{{ aboutMe.gender }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="font-medium">Nationality</span><span>{{ aboutMe.nationality }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="font-medium">Status</span><span>{{ aboutMe.status }}</span>
                  </div>
                </div>
              </section>
              <section
                class="bg-white p-6 rounded-2xl shadow-md page-break-before"
                role="region"
                aria-labelledby="work-title"
              >
                <h3 id="work-title" class="text-xl font-semibold mb-6 text-gray-900 border-b pb-3">
                  Work Experience
                </h3>
                <div class="space-y-8 relative before:absolute before:left-3 before:top-0 before:bottom-0 before:w-0.5 before:bg-gray-200 before:z-0">
                  <div v-for="exp in workExperience" :key="exp.title" class="relative pl-10 z-10 page-break-inside">
              ...
              >
                <h3 id="workshops-title" class="text-xl font-semibold mb-6 text-gray-900 border-b pb-3">
                  Technical Workshops
                </h3>
                <div class="space-y-6">
                  <div v-for="workshop in workshops" :key="workshop.title" class="p-4 rounded-lg border border-gray-100 shadow-sm bg-white page-break-inside">
                    <div class="flex justify-between items-baseline mb-1">
                      <h4 class="font-semibold text-gray-900">{{ workshop.title }}</h4>
                      <div class="text-sm text-gray-500">{{ workshop.date }}</div>
                    </div>
                    <div v-if="workshop.facilitator" class="text-sm text-gray-600 mb-3">
              ...
              >
                <h3 id="references-title" class="text-xl font-semibold mb-6 text-gray-900 border-b pb-3">
                  References
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div v-for="ref in references" :key="ref.email" class="p-4 border rounded-lg shadow-sm bg-white page-break-inside">
                    <div class="font-semibold text-gray-900 mb-1">{{ ref.name }}</div>
                    <div class="text-sm text-gray-600 mb-3">{{ ref.position }}</div>
                    <div class="text-sm text-gray-700 flex items-center gap-2">
                      <span class="font-medium">Phone:</span> {{ ref.phone }}
                    </div>
                    <div class="text-sm text-gray-700 flex items-center gap-2">
                      <span class="font-medium">Email:</span> {{ ref.email }}
                    </div>
                  </div>
                </div>
              </section>
...
<style scoped>
/* Letter size dimensions: 215.9mm x 279.4mm (8.5in x 11in) */
.letter-container {
  width: 215.9mm;
  margin: 0 auto;
  background: white;
...
/* Ensure SVGs and other elements are visible in print */
svg {
...
/* Ensure progress bars are rendered clearly */
.bg-gradient-to-r {
  background: #4b6cb7 !important; /* Fallback for PDF */
}
